...
test('VFX Draw Call Minimization: InstancedMesh consolidation for shockwave rings & storm radar cells', () => {
  ...
  assert.equal(radar.instancedHexMesh.count, 512, 'instancedHexMesh must support 512 cells');
  ...
  assert.equal(radar.group.children.filter(c => c === radar.instancedHexMesh).length, 1, 'Only 1 consolidated InstancedMesh for 512 cells');
